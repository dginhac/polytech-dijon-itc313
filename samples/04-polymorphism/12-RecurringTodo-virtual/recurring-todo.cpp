...
bool RecurringTodo::isOverdue(const Date& date) const  {
    return !isDone()
        && endDate() < date;   // simplified for teaching
}

// Free functions

std::ostream& operator<<(std::ostream& os, const RecurringTodo& rtodo) {
    os << static_cast<const Todo&>(rtodo);
    // Add recurrence details
    os << std::string(" — every ") << rtodo.periodDays() << std::string(" days");
    os << std::string(" until ") << rtodo.endDate();

    // If the series is finished, make it explicit
    if (rtodo.isDone()) {
        os << std::string(" [series complete]");
    }
    return os;
}
